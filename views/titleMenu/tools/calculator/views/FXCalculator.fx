 ...
 * FXCalculator.fx
 *
 * Created on Feb 20, 2010, 9:49:47 PM
 */
...
public class FXCalculator extends CustomNode {
    ...
    var bgRect = Rectangle {
        x: 3 y: 3 width: width height: height
        fill: backFill
        stroke: lineFill
        strokeWidth: 3.0
        arcWidth: 20
    ...
    var numberText : Text = Text {
        translateX: bind width - numberText.boundsInLocal.width - 19
        translateY: 13 + border
        font: Font { name: "monospaced" size: 25 }
        fill: Color.BLACK
    ...
    var operationText : Text = Text {
        translateX: 8 + border
        translateY: 13 + border
        font: Font { name: "monospaced" size: 10 }
        fill: Color.BLACK
    ...
    override function create() : Node {
        ...
        return Group {
            content:  [
                bgRect, numberBackground, numberText, operationText, memoryText,
                Group {
                    translateY: transY
                    content: [ calcKeys ]
                }
    ...
    function onDecimal() {
        ...
        // Reset the display
        if(reset) { number = "0"; reset = false; }

        if(not decimal) { number = "{number}."; }
        decimal = true;
    }
    ...
    function setSelKey(index : Integer) {
        for(key in calcKeys) { key.selected = false; }
